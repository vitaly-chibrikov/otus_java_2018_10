...
public class MutationExamples {
    ...
    private List<TestObjectUnMutable> listUnMute = Arrays.asList(new TestObjectUnMutable(1), new TestObjectUnMutable(2), new TestObjectUnMutable(3));
    ...
    private void mutableUnMutable() {
        ...
        List<TestObjectMutable> newList = listMute.stream().map(elem -> elem.updateValue(-1)).collect(Collectors.toList());
        System.out.println(newList);

        //Лучше так - создаем новый экземпляр, если "надо бы поменять существующий"
        List<TestObjectUnMutable> newList2 = listUnMute.stream().map(elem -> elem.updateValue(-1)).collect(Collectors.toList());
